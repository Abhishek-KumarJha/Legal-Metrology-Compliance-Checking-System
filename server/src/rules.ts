...
export const rules: Rule[] = [
  ...
  { id: 'netQuantity', fieldName: 'netQuantity', label: 'Net quantity', pattern: /(?:(?:NETT?|NET|CONTENTS?|QUANTITY)\s*(?:WEIGHT|WT|QUANTITY|QTY|CONTENTS?)?\s*[:.-]?\s*([0-9OIl]+(?:[.,-][0-9OIl]+)?)\s*(kg|litre|gm|mg|ml|g|l|n|nos|pieces?|tablets?)\b|(?:^|\n)\s*([0-9OIl]+(?:[.,-][0-9OIl]+)?)\s*(kg|litre|gm|mg|ml|g|l|n|nos|pieces?|tablets?)\b)/i, regexPattern: 'Net/Nett weight, quantity, contents, or standalone count/quantity with unit', ruleSection: 'Rule 6(1)(d)', minFontSizeMm: 2, description: 'Standard units, counts, and numerals', expectedPanel: 'declarations' },
  // Dates appear as MFG/PKD, manufacturing or packed-on text, and may be read before their label.
  { id: 'date', fieldName: 'date', label: 'Date marking', pattern: /(?:(?:MFG|MFD|MANUF|PKD|PACKED|PACKED\s*ON|MANUFACTURED|MANUFACTURING\s*DATE|DATE\s*OF\s*(?:MANUFACTURE|PACKING)|BEST\s*BEFORE|USE\s*BY|EXP(?:IRY)?)\s*[:.-]?[\s\S]{0,80}?([0-9]{1,2}\s+[A-Z]{3}\s+[0-9]{2,4}|[0-9]{1,2}[\/.-][0-9]{1,2}[\/.-][0-9]{2,4}|[0-9]{4}[\/.-][0-9]{1,2}(?:[\/.-][0-9]{1,2})?|[0-9]{1,2}[\/.-][0-9]{4}|[A-Z]{3}\s*[0-9]{4}|[A-Z]{3}[0-9]{4}|(?:one|two|three|four|five|six|seven|eight|nine|ten|twelve|[0-9]+)\s+(?:days?|months?|years?)\s+(?:from|after|of)\s+(?:the\s+)?(?:manufactur(?:e|ing)?|mfg|mfd|pack|packaging|pkd|open))|([0-9]{1,2}\s+[A-Z]{3}\s+[0-9]{2,4}|[0-9]{1,2}[\/.-][0-9]{1,2}[\/.-][0-9]{2,4}|[0-9]{4}[\/.-][0-9]{1,2}(?:[\/.-][0-9]{1,2})?|[0-9]{1,2}[\/.-][0-9]{4}|[A-Z]{3}\s*[0-9]{4}|[A-Z]{3}[0-9]{4}|(?:one|two|three|four|five|six|seven|eight|nine|ten|twelve|[0-9]+)\s+(?:days?|months?|years?)\s+(?:from|after|of)\s+(?:the\s+)?(?:manufactur(?:e|ing)?|mfg|mfd|pack|packaging|pkd|open))[\s\S]{0,80}?(?:MFG|MFD|MANUF|PKD|PACKED|MANUFACTURED|MANUFACTURING\s*DATE|BEST\s*BEFORE|USE\s*BY|EXP(?:IRY)?))/i, regexPattern: 'Absolute or word/number duration-based date marking near manufacturing, packing, best-before, use-by, or expiry context', ruleSection: 'Rule 6(1)(c)', minFontSizeMm: 2, description: 'Date, month, year, or duration marking', expectedPanel: 'declarations' },
  // Manufacturer wording covers common MFD/packed/marketed labels and captures the following line.
  { id: 'manufacturer', fieldName: 'manufacturer', label: 'Manufacturer / packer', pattern: /(?:MFD?\.?\s*BY|MFG\.?\s*BY|MANUFACTURED\s*BY|PACK(?:ED|ER)\s*BY|MARKETED\s*BY|IMPORTED\s*BY|MANUFACTURER)\s*[:.-]?\s*([^\n]+)/i, regexPattern: 'Manufacturer, packer, marketer, importer, or associated address block', ruleSection: 'Rule 6(1)(a)', minFontSizeMm: 2, description: 'Name and address of manufacturer, packer or importer', expectedPanel: 'declarations' },
