...
export type ReportInput = {
  ...
  readability?: { readable: boolean; note: string; minimumWordHeightPx: number | null };
...
export async function generateCompliancePdf(input: ReportInput, outputDirectory: string) {
  ...
  if (input.ocrFallbackUsed) write('DEMO MODE: canned OCR fallback used; no live OCR text was available.', 9, bold, rgb(0.65, 0.42, 0.08));
  if (input.readability) write(`Readability estimate: ${input.readability.readable ? 'readable' : 'review required'}; ${input.readability.minimumWordHeightPx === null ? 'text height unavailable' : `smallest recognized text ${input.readability.minimumWordHeightPx} px`}. Not calibrated in millimetres.`, 9, regular, rgb(0.4, 0.42, 0.39));
  write('Rule 9 legibility signals (estimated from OCR geometry; advisory, not a font measurement)', 9, bold, rgb(0.4, 0.42, 0.39));
  y -= 5;
  ...
  for (const check of input.checks) {
    ...
    y -= 19;
    page.drawText(check.status === 'manual-verified' ? 'Verification source: Officer manually verified' : check.status === 'verified' ? 'Verification source: OCR-verified' : 'Verification source: Review required', { x: 190, y, size: 8, font: bold, color: check.status === 'manual-verified' ? rgb(0.35, 0.25, 0.08) : rgb(0.25, 0.3, 0.28) });
    y -= 13;
    ...
    if (check.legibility) {
      page.drawText(`Legibility: font ${check.legibility.fontSizeConsistent === null ? 'unable to assess' : check.legibility.fontSizeConsistent ? 'consistent' : 'inconsistent'}; spacing ${check.legibility.spacingIssue === null ? 'unable to assess' : check.legibility.spacingIssue ? 'issue' : 'normal'}.`, { x: 190, y, size: 7, font: regular, color: rgb(0.4, 0.42, 0.39) });
      y -= 13;
      page.drawText(`Style signal: ${check.legibility.styleSignal.slice(0, 82)}`, { x: 190, y, size: 7, font: regular, color: rgb(0.4, 0.42, 0.39) });
      y -= 13;
    }
  }
  y -= 10;
